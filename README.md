...
> **穩定版聲明**：本版（**v0.1.3**，2026-09-26）為新增「軟體更新（GitHub 版本檢查／下載）」後、本機測試通過的穩定版。
...
## 本版（v0.1.3，2026-09-26）

### 軟體更新（自動檢查版本）

- 還原「軟體更新」介面：顯示「立刻檢查」按鈕、「啟動時檢查」勾選框、檢查時間與版本資訊欄位。
- 版本檢查改用 GitHub：讀取 `version.json`，取代失效的 Yahoo 舊伺服器。
- 檢查到新版時，出現「下載更新」按鈕：
  - 按下即直接下載最新 `.pkg` 到 `~/Downloads`，並自動開啟安裝器（免再自行雙擊）。
  - 下載失敗時，自動開啟 GitHub 發布頁（fallback）。
- 支援「啟動時檢查」（勾選後啟動即自動檢查）。

### 版本資訊與安裝畫面

- 「您目前使用的版本」顯示 `1.1.2535-arm64(0.1.3)`。
- 安裝器第一畫面顯示「安裝「Yahoo! KeyKey 0.1.3」」；pkg 版本識別為 `0.1.3`。
- 「上次檢查更新的時間」完整顯示（含時、分、秒）。
