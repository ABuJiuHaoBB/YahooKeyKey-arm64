...
- **改用「小麥注音（McBopomofo）」詞庫重建語言模型，大幅提升選字精準度**：
  ...
  - 改用 McBopomofo 的開源詞庫（`BPMFBase.txt` 單字映射、`BPMFMappings.txt`
    ...
    - unigrams（27102 筆，含標點）：單字→絕對順序編碼，機率 = log(頻率+1)。
    - bigrams（137501 筆）：片語相鄰字對，機率 = log(頻率+1) + 8.0（讓上下文優先）。
  - **選字精準度（自動校正）修正**：
    - 把 unigram 頻率改為「依片語歸屬讀音」（避免多音字把常用讀音誤套到罕用讀音），
      例：`ㄏㄨㄛˋ` 正確選「或」（或許）而非「和」。
    - 組字器 `fastWalk` 改為「探索所有候選路徑 + beam 限制」（非貪婪），
      例：`ㄕˋ ㄎㄢˋ ㄎㄢˋ` 正確選「試看看」而非「是看看」。
  - **標點符號修正**：把 `bpmf-punctuations.cin` 的 `_punctuation_*` 條目加入
    unigrams 表，使「好打注音」能如傳統注音般用 shift+標點打出全形符號
    （例如 shift+`,` → `，`、shift+`.` → `。`）。
  - 已驗證：`ㄊㄞˊ ㄨㄢ`→「台灣」、`ㄏㄨㄛˋ ㄒㄩˇ`→「或許」、
    `ㄕˋ ㄎㄢˋ ㄎㄢˋ`→「試看看」、shift+`,`→`，`。
